...
%------------------------------------------------
\subsection{Solving nonlinear separable classification problems}

\begin{frame}{}
	\frametitle{A suitable kernel}
	Going back to our problem of nonlinear separable data, we can use a kernel function of the form
    \begin{equation}
        k(x, x') = exp \left( - \frac{\left\lVert x - x' \right\rVert^2}{2 \sigma^2} \right),
    \end{equation}
    a so called \textbf{Gaussian radial basis function} with $ \sigma > 0$.
\end{frame}
